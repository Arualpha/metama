...
export default function App() {
  ...
  const backButtonStyle = {
    ...
    fontWeight: 'bold',
    cursor: 'pointer',
    boxShadow: '0 4px 10px rgba(239, 75, 129, 0.2)',
    fontSize: '0.9rem'
  };
  ...
  return (
    <div style={wrapperStyle}>
      ...
      {page === 'gifts' && !selectedGift && (
        <div style={{ ...cardStyle, maxWidth: '550px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', width: '100%' }}>
            ...
            <div onClick={() => setSelectedGift('gallery')} style={{ cursor: 'pointer', flex: 1 }}><img src={gift3} style={{ width: '100%', borderRadius: '15px' }} alt="Gallery" /></div>
          </div>
        </div>
      )}
      ...
      {page === 'gifts' && selectedGift === 'letter' && (
        <div style={cardStyle}>
          <h2 style={{ color: '#5c2d3a', marginBottom: '20px' }}>A Message for You ❤️</h2>
          <div style={{ border: '2px dashed #ef4b81', padding: '20px', borderRadius: '20px', backgroundColor: '#fff5f7' }}>
            <img src={gift2} style={{ width: '80px', marginBottom: '10px' }} alt="Letter icon" />
            <p style={{ color: '#4a4a4a', fontStyle: 'italic' }}>"Every moment with you feels like a dream come true. You're my heart, my soul, and my favorite person. Happy Valentine's Day, Patootie!"</p>
          </div>
      ...
      {page === 'gifts' && selectedGift === 'gallery' && (
        <div style={{ ...cardStyle, maxWidth: '600px' }}>
          <h2 style={{ color: '#5c2d3a', marginBottom: '20px' }}>Our Precious Moments 📸</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', maxHeight: '400px', overflowY: 'auto', padding: '10px' }}>
            {photos.map((p, i) => (
              <div key={i} style={{ position: 'relative' }}>
                <img src={p.img} style={{ width: '100%', borderRadius: '15px' }} alt={p.caption} />
